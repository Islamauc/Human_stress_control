...
static HAL_StatusTypeDef reg_read(I2C_HandleTypeDef *hi2c, uint8_t reg, uint8_t *val)
{
...
HAL_StatusTypeDef MAX30102_ReadRaw(I2C_HandleTypeDef *hi2c, uint32_t *red, uint32_t *ir)
{
    uint8_t wr_ptr, rd_ptr;
    if (reg_read(hi2c, REG_FIFO_WR_PTR, &wr_ptr) != HAL_OK) 
			return HAL_ERROR;
    if (reg_read(hi2c, REG_FIFO_RD_PTR, &rd_ptr) != HAL_OK) 
			return HAL_ERROR;
    if (wr_ptr == rd_ptr) 
			return HAL_ERROR;

    uint8_t buf[6];
    if (HAL_I2C_Mem_Read(hi2c, MAX_ADDR, REG_FIFO_DATA, 1, buf, 6, 50) != HAL_OK) return HAL_ERROR;

    *red = ((uint32_t)(buf[0] & 0x03) << 16)| ((uint32_t) buf[1]  <<  8) | (uint32_t) buf[2];
    *ir  = ((uint32_t)(buf[3] & 0x03) << 16) | ((uint32_t) buf[4] <<  8) | (uint32_t) buf[5];
