...
/* Reject IBIs that deviate more than 40% from the running mean.
   Returns 1 if IBI is plausible, 0 if it should be discarded.   */
...
void HRV_Compute(HRV_t *h)
{
    ...
    if (h->ibi_ms > 0)
        h->hr_bpm = (uint8_t)(60000UL / h->ibi_ms);
    ...
    /* Realistic RMSSD ranges for stress mapping:
       Healthy resting RMSSD is typically 20-80ms.
       >80ms = very relaxed, <10ms = very stressed.
       We cap at 80ms so the scale is meaningful.       */
    if (h->rmssd_ms >= 80)
        h->stress_index = 0;
